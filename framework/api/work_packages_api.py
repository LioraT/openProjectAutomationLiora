...
class WorkPackagesApi(RestClient):
    ...
    def get_work_package(self, work_package_id, expected_response_status_code: int = HTTPStatus.OK):
        response = self.get(path=f"/api/v3/work_packages/{work_package_id}",
                            basic_auth=self.basic_auth,
                            expected_response_status_code=expected_response_status_code)
        return response.json()

    def create_work_package(self, payload):
        response = self.post(path=f"/api/v3/work_packages", payload=payload,
    ...
    def delete_work_package(self, work_package_id):
        self.delete(path=f"/api/v3/work_packages/{work_package_id}",
                    basic_auth=self.basic_auth, content_type=self.content_type,
